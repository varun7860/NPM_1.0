...
class Npm_1(object):
    ...
    def arrive_at_sorting_loc(self):
        ...
        self.set_joint_angles([1.57,-1.57])
        #self.set_joint_angles([0.7,1.7])
        #self.set_joint_angles([3.14,0.0])
...
class NpmPusher(Npm_1):
    ...
    def increase_level(self):
        ...
        while(flag_plan != True):
            joint_value = self._group_2.get_current_joint_values()
            self._group_2.set_joint_value_target([joint_value[0] - 0.005])
            flag_plan = self._group_2.go(wait=True)
                ...
                if val not in LEVEL_CURVES:
                    ...
                    if len(LEVEL_CURVES) > 0:
                        ...
                        for k in LEVEL_CURVES:
                              diff = abs(k-val)
                              if diff <= 0.025 or val < 0.81 or val == 1.17:
                                 all_curves = False
                                 break
...
def main():
    ...
    pusher.store_value(10)

    pusher.sweep(-30)
    robot.go_to_defined_pose('up','planner_2')

    #print("\033[1;31m Robot Initialized " + '\033[0m')
